...
describe("Utils", () => {
  describe("promisify()", () => {
    ...
    it("should resolve successes", () => {
      return promisifiedAsyncFn("a string").then(data =>
        expect(data).toBe(successMsg)
      );
    });
    it("should reject errors", () => {
      return promisifiedAsyncFn(["not", "a", "string"]).catch(err =>
  ...
  describe("memoize()", () => {
    ...
    it("should not collide when bound instance methods are passed", () => {
      ...
      expect(expected).not.toBe(
        util.memoize(Map.prototype.get.bind(dummy), "foo")
      ); // dummy.get('foo') // => 'definitely not bar'
